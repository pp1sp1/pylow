...
class ExternalDeclarationsMixin:
    ...
    def _init_time_module(self):
        """Initialize time module functions."""
        # time() function - returns current time as double
        fty = ir.FunctionType(F64, [])
        fn = ir.Function(self.module, fty, name="time")
        self.functions["time.time"] = fn

    def _init_os_module(self):
        """Initialize os module functions."""
        # getcwd() - get current working directory
        fty = ir.FunctionType(I8P, [])
        fn = ir.Function(self.module, fty, name="getcwd")
        self.functions["os.getcwd"] = fn
