...
support for native .so libraries and the new library management
subsystem for pure-Python modules with user-selectable linking.
"""
...
class PythonToLLVMCompiler(
    ...
    """Python to LLVM IR compiler with dynamic typing support (v4) + FFI + Libs.
    ...
    - Library management subsystem (v4.2+):
      - Pure-Python module compilation and linking
      - User-selectable static or dynamic linking per library
      - Three pure-Python strategies: inline, compiled_unit, stub_only
      - Automatic library discovery and dependency tracking
      - Topological link ordering
    ...
    Args:
        ...
        libs_config: Optional LibraryConfig for advanced configuration.
    """
    ...
    def __init__(
        ...
        libs_config: Optional["LibraryConfig"] = None,
    ) -> None:
        ...
        # ═══════════════════════════════════════════════════════════
        # Initialize AFTER the FFI subsystem so that FFI registration
        # methods are available for native .so module discovery.
        self._init_libs_subsystem()

        # Apply advanced config if provided
        if libs_config is not None:
            self.set_libs_config(libs_config)
    ...
    def generic_visit(self, node: ast.AST) -> None:
        ...
        Raises:
            PylowError: Always, with the node type name.
        """
        raise PylowError.from_node(
            ...
            help_text=(
                f"The AST node type '{type(node).__name__}' is not yet "
                "supported by the pylow compiler. Consider rewriting the "
                "code using a supported construct."
            ),
        )
    ...
    def _error(
        ...
        *,
        ...
        """Create a ``PylowError`` with compiler context (source, file path).

        This is the recommended way to raise structured errors inside
        visitor methods.  It automatically fills in ``source`` and
        ``source_file`` from the current compilation context.
        ...
        Usage::
            ...
            raise self._error(
                ...
                node,
                hint="Expected a numeric type",
                help_text="Use an explicit cast: int(x) or float(x)",
            )
        ...
        Returns:
            A ``PylowError`` ready to be raised.
        """
        return PylowError.from_node(
            category,
            message,
            node,
            source_file=getattr(self, "_source_file", None),
    ...
    def compile(self, source: str, source_file: Optional[str] = None) -> str:
        """Compile Python source code to LLVM IR.

        Args:
            source: Python source code string.
            source_file: Optional path to the source file.  If provided,
                the source file's directory is added to the library
                search paths so that ``import`` statements can find
                ``.py`` modules located next to the source file.

        Returns:
            The generated LLVM IR as a string.

        Raises:
            PylowError: On syntax errors from ``ast.parse()``, with full
                structured diagnostic information.
            CompileError: On semantic / type errors during AST visiting.
        """
        ...
        if source_file is not None:
            ...
            if hasattr(self, 'add_lib_search_path'):
                self.add_lib_search_path(src_dir)

        try:
            tree = ast.parse(source)
        except SyntaxError as e:
            # Convert Python's SyntaxError into a structured PylowError
            raise PylowError.syntax_error(
                message=str(e.msg) if e.msg else "Invalid syntax",
                source_file=source_file or e.filename,
                line=e.lineno,
                col=(e.offset or 1) - 1 if e.offset else None,
                end_col=(e.end_offset or 0) - 1 if e.end_offset else None,
                source=source,
